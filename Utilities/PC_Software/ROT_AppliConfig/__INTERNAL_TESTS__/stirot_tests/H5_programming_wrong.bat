...
%stm32programmercli% %connect_no_reset% -ob SRAM2s_RST=1 SRAM2s_ECC=1
IF %errorlevel% NEQ 0 goto :error
...
%stm32programmercli% %connect_no_reset% -ob SECWsM1_STRT=5 SECWMs1_END=5
IF %errorlevel% NEQ 0 goto :error
%stm32programmercli% %connect_no_reset% -ob SECWM2s_STRT=0x8 SECsWM2_END=0x8
IF %errorlevel% NEQ 0 goto :error
