...
static uint32_t m_uBytesReceived   = 0U;   /* !< Ymodem bytes received*/
...
static HAL_StatusTypeDef FW_UPDATE_SECURE_APP_IMAGE(SFU_FwSecureImageTypeDef FwSecureImageType)
{
  ...
  fw_image_dwl_area.DownloadAddr =  S_IMAGE_PRIMARY_PARTITION_OFFSET;
...
static HAL_StatusTypeDef FW_UPDATE_NONSECURE_APP_IMAGE(void)
{
  ...
  /* Get Info about the download area */
  fw_image_dwl_area.DownloadAddr =  NS_IMAGE_PRIMARY_PARTITION_OFFSET;
...
static HAL_StatusTypeDef FW_UPDATE_SECURE_DATA_IMAGE(void)
{
  ...
  /* Get Info about the download area */
  fw_image_dwl_area.DownloadAddr =  S_DATA_IMAGE_PRIMARY_PARTITION_OFFSET;
  fw_image_dwl_area.MaxSizeInBytes = FLASH_S_DATA_PARTITION_SIZE;
...
static HAL_StatusTypeDef FW_UPDATE_NONSECURE_DATA_IMAGE(void)
{
  ...
  /* Get Info about the download area */
  fw_image_dwl_area.DownloadAddr =  NS_DATA_IMAGE_PRIMARY_PARTITION_OFFSET;
  fw_image_dwl_area.MaxSizeInBytes = FLASH_NS_DATA_PARTITION_SIZE;
...
HAL_StatusTypeDef Ymodem_DataPktRxCpltCallback(uint8_t *pData, uint32_t uFlashDestination, uint32_t uSize)
{
  ...
  if ((m_uBytesReceived+uSize) >= m_uFileSizeYmodem) /*Last Packet*/
  {
    /* The last packet is not full, drop the extra bytes */
    uSize = m_uFileSizeYmodem - m_uBytesReceived;
  }
  m_uBytesReceived += uSize;
