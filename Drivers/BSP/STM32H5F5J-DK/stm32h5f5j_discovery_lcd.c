/**
  ******************************************************************************
  * @file    stm32h5f5j_discovery_lcd.c
  * @author  MCD Application Team
  * @brief   This file includes the driver for Liquid Crystal Display (LCD) module
  *          mounted on stm32h5f5j_DK board.
  ******************************************************************************
  * @attention
  *
  ...
  ******************************************************************************
  ...
  --------------------------
   ...
   - This driver uses the adequate timing and setting for the ST1633i LCD component
  ...
  --------
    ...
    an LCD instance. On the stm32h5f5j Discovery board, there's one instance. Then, this
...
/** @defgroup stm32h5f5j_DK_LCD LCD
  * @{
  */

/** @defgroup stm32h5f5j_DK_LCD_Private_Variables LCD Private Variables
  * @{
  */
...
/** @defgroup stm32h5f5j_DK_LCD_Private_TypesDefinitions LCD Private TypesDefinitions
  * @{
  */
...
/** @defgroup stm32h5f5j_DK_LCD_Exported_Variables LCD Exported Variables
  * @{
  */
...
/** @defgroup stm32h5f5j_DK_LCD_Private_FunctionPrototypes LCD Private FunctionPrototypes
  * @{
  */
...
/**
  * @}
  */
/** @defgroup stm32h5f5j_DK_LCD_Private_Macros  Private Macros
  * @{
  */
...
/** @defgroup stm32h5f5j_DK_LCD_Exported_Functions LCD Exported Functions
  * @{
  */
/**
...
__weak HAL_StatusTypeDef MX_LTDC_Init(LTDC_HandleTypeDef *hltdc, uint32_t Width, uint32_t Height)
{
  ...
  hltdc->Init.HorizontalSync     = (uint32_t)ST1633I_HSYNC - 1U;
  hltdc->Init.AccumulatedHBP     = (uint32_t)ST1633I_HSYNC + (uint32_t)ST1633I_HBP - 1U;
  hltdc->Init.AccumulatedActiveW = (uint32_t)ST1633I_HSYNC + Width + (uint32_t)ST1633I_HBP -1U;
  hltdc->Init.TotalWidth         = (uint32_t)ST1633I_HSYNC + Width + (uint32_t)ST1633I_HBP + ST1633I_HFP - 1U;
  hltdc->Init.VerticalSync       = (uint32_t)ST1633I_VSYNC - 1U;
  hltdc->Init.AccumulatedVBP     = (uint32_t)ST1633I_VSYNC + (uint32_t)ST1633I_VBP - 1U;
  hltdc->Init.AccumulatedActiveH = (uint32_t)ST1633I_VSYNC + Height + (uint32_t)ST1633I_VBP -1U;
  hltdc->Init.TotalHeigh         = (uint32_t)ST1633I_VSYNC + Height + (uint32_t)ST1633I_VBP + ST1633I_VFP - 1U;
...
__weak HAL_StatusTypeDef MX_LTDC_ClockConfig(LTDC_HandleTypeDef *hltdc)
{
  ...
  /* hltdc unused argument(s) compilation warning */
  UNUSED(hltdc);
  /* LTDC frequency = 9MHz*/
  RCC_PeriphCLKInitStruct.PeriphClockSelection = RCC_PERIPHCLK_LTDC;
  RCC_PeriphCLKInitStruct.LtdcClockSelection = RCC_LTDCCLKSOURCE_PLL2R;
  RCC_PeriphCLKInitStruct.PLL2.PLL2M = 40;
  RCC_PeriphCLKInitStruct.PLL2.PLL2N = 129;
  RCC_PeriphCLKInitStruct.PLL2.PLL2P = 2;
  RCC_PeriphCLKInitStruct.PLL2.PLL2Q = 2;
  RCC_PeriphCLKInitStruct.PLL2.PLL2R = 16;
  RCC_PeriphCLKInitStruct.PLL2.PLL2FRACN = 0;
  RCC_PeriphCLKInitStruct.PLL2.PLL2RGE =  RCC_PLLVCIRANGE_1;
  RCC_PeriphCLKInitStruct.PLL2.PLL2ClockOut = RCC_PLL2_DIVR ;
...
int32_t BSP_LCD_FillRGBRect(uint32_t Instance, uint32_t Xpos, uint32_t Ypos, uint8_t *pData, uint32_t Width, uint32_t Height)
{
  ...
  if(Lcd_Ctx[Instance].PixelFormat == LCD_PIXEL_FORMAT_RGB888)
  {
    for (i = 0; i < Height; i++)
    {
      for (j = 0; j < Width; j++)
...
static void LTDC_MspInit(LTDC_HandleTypeDef *hltdc)
{
  ...
  if (hltdc->Instance == LTDC)
  {
    ...
    */
    ...
    gpio_init_structure.Pin       = GPIO_PIN_2 | GPIO_PIN_7 | GPIO_PIN_10 ;
    gpio_init_structure.Alternate = GPIO_AF14_LTDC;
    HAL_GPIO_Init(GPIOB, &gpio_init_structure);

    gpio_init_structure.Pin       = GPIO_PIN_0 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_11;
    gpio_init_structure.Alternate = GPIO_AF13_LTDC;
    HAL_GPIO_Init(GPIOD, &gpio_init_structure);

    gpio_init_structure.Pin       = GPIO_PIN_3 | GPIO_PIN_11 ;
    gpio_init_structure.Alternate = GPIO_AF14_LTDC;
    gpio_init_structure.Pin       = GPIO_PIN_4 ;
    gpio_init_structure.Alternate = GPIO_AF10_LTDC;
    HAL_GPIO_Init(GPIOE, &gpio_init_structure);

    gpio_init_structure.Pin       =GPIO_PIN_13 | GPIO_PIN_14 ;
    gpio_init_structure.Alternate = GPIO_AF13_LTDC;
    ...
    gpio_init_structure.Pin       = GPIO_PIN_5 | GPIO_PIN_11 ;
    gpio_init_structure.Alternate = GPIO_AF14_LTDC;
    HAL_GPIO_Init(GPIOI, &gpio_init_structure);

    gpio_init_structure.Pin       = GPIO_PIN_4 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11;
    gpio_init_structure.Alternate = GPIO_AF14_LTDC;
    HAL_GPIO_Init(GPIOJ, &gpio_init_structure);

    gpio_init_structure.Pin       = GPIO_PIN_11 | GPIO_PIN_12| GPIO_PIN_13| GPIO_PIN_15 ;
    gpio_init_structure.Alternate = GPIO_AF14_LTDC;
    HAL_GPIO_Init(GPIOK, &gpio_init_structure);

    /* LCD_ON, LCD_BL_CTRL */
    gpio_init_structure.Pin       = GPIO_PIN_3| GPIO_PIN_8;
    gpio_init_structure.Mode      = GPIO_MODE_OUTPUT_PP;
    HAL_GPIO_Init(GPIOF, &gpio_init_structure);
