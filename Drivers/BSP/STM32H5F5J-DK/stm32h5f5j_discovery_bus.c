/**
  ******************************************************************************
  * @file    stm32h5f5j_discovery_bus.c
  * @author  MCD Application Team
  * @brief   This file provides a set of firmware functions to communicate
  *          with  external devices available on STM32H5F5J-DK board (MB2114) from
  *          STMicroelectronics
  ******************************************************************************
  * @attention
  *
...
/* Includes ------------------------------------------------------------------*/
#include "stm32h5f5j_discovery_bus.h"
...
/** @defgroup STM32H5F5J_DK_BUS BUS
  * @{
  */

/** @defgroup STM32H5F5J_DK_BUS_Private_Constants Private Constants
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_BUS_Private_Types Private Types
  * @{
  */
typedef struct
{
...
/** @addtogroup STM32H5F5J_DK_BUS_Private_Constants
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_BUS_Private_Variables Private Variables
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_BUS_Exported_Variables Exported Variables
  * @{
  */
I2C_HandleTypeDef hbus_i2c1 = {0};
I2C_HandleTypeDef hbus_i2c4 = {0};
/**
  * @}
  */

/** @defgroup STM32H5F5J_DK_BUS_Private_FunctionPrototypes BUS Private FunctionPrototypes
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_BUS_Exported_Functions Exported Functions
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_BUS_Private_Functions Private Functions
  * @{
  */
/**
...
int32_t BSP_I2C4_Init(void)
{
  ...
  if (I2c4InitCounter == 0U)
  {
    if (HAL_I2C_GetState(&hbus_i2c4) == HAL_I2C_STATE_RESET)
    {
      ...
      if (MX_I2C4_Init(&hbus_i2c4, I2C_GetTiming(HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_I2C4),
                                                 BUS_I2C4_FREQUENCY)) != HAL_OK)
      {
        ret = BSP_ERROR_BUS_FAILURE;
      }
#else
  ...
  if (I2c4InitCounter < 0xFFFFFFFFU)
  {
    I2c4InitCounter++;
  }
...
{
  int32_t ret = BSP_ERROR_NONE;
  if (HAL_I2C_Master_Transmit(&hbus_i2c4, DevAddr, pData, Length, BUS_I2C4_TIMEOUT) != HAL_OK)
  {
    if (HAL_I2C_GetError(&hbus_i2c4) != HAL_I2C_ERROR_AF)
    {
      ret = BSP_ERROR_BUS_ACKNOWLEDGE_FAILURE;
...
int32_t BSP_I2C4_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t Length) {
  int32_t ret = BSP_ERROR_NONE;
  if (HAL_I2C_Master_Receive(&hbus_i2c4, DevAddr, pData, Length, BUS_I2C4_TIMEOUT) != HAL_OK)
  {
    if (HAL_I2C_GetError(&hbus_i2c4) != HAL_I2C_ERROR_AF)
    {
      ret = BSP_ERROR_BUS_ACKNOWLEDGE_FAILURE;
...
int32_t BSP_I2C4_IsReady(uint16_t DevAddr, uint32_t Trials)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif /* BSP_USE_CMSIS_OS */
  if (HAL_I2C_IsDeviceReady(&hbus_i2c4, DevAddr, Trials, BUS_I2C4_TIMEOUT) != HAL_OK)
  {
    ret = BSP_ERROR_BUSY;
  }
#if defined(BSP_USE_CMSIS_OS)
...
static int32_t I2C4_WriteReg(uint16_t DevAddr, uint16_t Reg, uint16_t MemAddSize, uint8_t *pData, uint16_t Length)
{
  int32_t  status = BSP_ERROR_NONE;
  uint32_t hal_error;
  if (HAL_I2C_Mem_Write(&hbus_i2c4, DevAddr, Reg, MemAddSize, pData, Length, BUS_I2C4_TIMEOUT) != HAL_OK)
  {
    hal_error = HAL_I2C_GetError(&hbus_i2c4);
...
static int32_t I2C4_ReadReg(uint16_t DevAddr, uint16_t Reg, uint16_t MemAddSize, uint8_t *pData, uint16_t Length)
{
  int32_t  status = BSP_ERROR_NONE;
  uint32_t hal_error;
  if (HAL_I2C_Mem_Read(&hbus_i2c4, DevAddr, Reg, MemAddSize, pData, Length, BUS_I2C4_TIMEOUT) != HAL_OK)
  {
    hal_error = HAL_I2C_GetError(&hbus_i2c4);
    if ((hal_error & HAL_I2C_ERROR_BERR) != 0U)
    {
