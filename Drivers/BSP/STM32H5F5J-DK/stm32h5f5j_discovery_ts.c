/**
  ******************************************************************************
  * @file    stm32h5f5j_discovery_ts.c
  * @author  MCD Application Team
  * @brief   This file provides a set of functions needed to manage the Touch
  *          Screen on STM32H5F5J-DK board.
  ******************************************************************************
  * @attention
  *
  ...
  ******************************************************************************
  ...
  ==============================================================================
   - This driver is used to drive the touch screen module of the STM32H5F5J-DK
...
/** @defgroup STM32H5F5J_DK_TS TS
  * @{
  */

/** @defgroup STM32H5F5J_DK_TS_Private_Defines TS Private Defines
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_TS_Private_Macros TS Private Macros
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_TS_Private_TypesDefinitions TS Private TypesDefinitions
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_TS_Privates_Variables TS Privates Variables
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_TS_Exported_Variables TS Exported Variables
  * @{
  */
...
/** @defgroup STM32H5F5J_DK_TS_Private_FunctionPrototypes TS Private Function Prototypes
  * @{
  */
static int32_t ST1633I_Probe(uint32_t Instance);
static void    TS_EXTI_Callback(void);
/**
  * @}
  */

/** @addtogroup STM32H5F5J_DK_TS_Exported_Functions
  * @{
  */
...
int32_t BSP_TS_Init(uint32_t Instance, TS_Init_t *TS_Init)
{
  ...
  {
    if (ST1633I_Probe(Instance) != BSP_ERROR_NONE)
    {
    ...
    else
    {
      ...
      else if(TS_Init->Orientation == TS_ORIENTATION_PORTRAIT)
      {
...
int32_t BSP_TS_GetState(uint32_t Instance, TS_State_t *TS_State)
{
  ...
  {
    ST1633I_State_t state;

    if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET)
    {
      /* Wait until INT PIN goes to high level mean that data of touch position is ready to read */
...
int32_t BSP_TS_Get_MultiTouchState(uint32_t Instance, TS_MultiTouch_State_t *TS_State)
{
  ...
  {
    ST1633I_MultiTouch_State_t state;

    /* A touch have beend detected through INT PIN */
    if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET)
    {
      /* Wait until INT PIN goes to high level mean that data of touch position is ready to read */
      while ((HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) != GPIO_PIN_SET));
...
/** @defgroup STM32H5F5J_DK_TS_Private_Functions TS Private Functions
  * @{
  */
/**
  * @brief  Probe the ST1633I TS driver.
  * @param  Instance TS Instance.
  * @retval BSP status.
  */
static int32_t ST1633I_Probe(uint32_t Instance)
{
  int32_t ret               = BSP_ERROR_NONE;
  ST1633I_IO_t              IOCtx;
  static ST1633I_Object_t   ST1633IObj;
  ...
  if (ST1633I_RegisterBusIO(&ST1633IObj, &IOCtx) != ST1633I_OK)
  {
    ret = BSP_ERROR_BUS_FAILURE;
  }
  else
  {
    Ts_CompObj[Instance] = &ST1633IObj;
    Ts_Drv = (TS_Drv_t *) &ST1633I_TS_Driver;
